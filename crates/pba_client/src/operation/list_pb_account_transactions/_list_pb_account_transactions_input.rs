// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ListPbAccountTransactionsInput  {
    #[allow(missing_docs)] // documentation missing in model
    pub account_id: ::std::option::Option<::std::string::String>,
    #[allow(missing_docs)] // documentation missing in model
    pub offset: ::std::option::Option<i64>,
    #[allow(missing_docs)] // documentation missing in model
    pub limit: ::std::option::Option<i64>,
    /// ISO 8601 date-time.
    pub from_date: ::std::option::Option<::aws_smithy_types::DateTime>,
    /// ISO 8601 date-time.
    pub to_date: ::std::option::Option<::aws_smithy_types::DateTime>,
}
impl  ListPbAccountTransactionsInput  {
    #[allow(missing_docs)] // documentation missing in model
    pub fn account_id(&self) -> ::std::option::Option<&str> {
        self.account_id.as_deref()
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn offset(&self) -> ::std::option::Option<i64> {
        self.offset
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn limit(&self) -> ::std::option::Option<i64> {
        self.limit
    }
    /// ISO 8601 date-time.
    pub fn from_date(&self) -> ::std::option::Option<&::aws_smithy_types::DateTime> {
        self.from_date.as_ref()
    }
    /// ISO 8601 date-time.
    pub fn to_date(&self) -> ::std::option::Option<&::aws_smithy_types::DateTime> {
        self.to_date.as_ref()
    }
}
impl ListPbAccountTransactionsInput {
    /// Creates a new builder-style object to manufacture [`ListPbAccountTransactionsInput`](crate::operation::list_pb_account_transactions::ListPbAccountTransactionsInput).
    pub fn builder() -> crate::operation::list_pb_account_transactions::builders::ListPbAccountTransactionsInputBuilder {
        crate::operation::list_pb_account_transactions::builders::ListPbAccountTransactionsInputBuilder::default()
    }
}

/// A builder for [`ListPbAccountTransactionsInput`](crate::operation::list_pb_account_transactions::ListPbAccountTransactionsInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ListPbAccountTransactionsInputBuilder {
    pub(crate) account_id: ::std::option::Option<::std::string::String>,
    pub(crate) offset: ::std::option::Option<i64>,
    pub(crate) limit: ::std::option::Option<i64>,
    pub(crate) from_date: ::std::option::Option<::aws_smithy_types::DateTime>,
    pub(crate) to_date: ::std::option::Option<::aws_smithy_types::DateTime>,
}
impl ListPbAccountTransactionsInputBuilder {
    #[allow(missing_docs)] // documentation missing in model
    /// This field is required.
    pub fn account_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.account_id = ::std::option::Option::Some(input.into());
        self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn set_account_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.account_id = input; self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn get_account_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.account_id
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn offset(mut self, input: i64) -> Self {
        self.offset = ::std::option::Option::Some(input);
        self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn set_offset(mut self, input: ::std::option::Option<i64>) -> Self {
        self.offset = input; self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn get_offset(&self) -> &::std::option::Option<i64> {
        &self.offset
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn limit(mut self, input: i64) -> Self {
        self.limit = ::std::option::Option::Some(input);
        self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn set_limit(mut self, input: ::std::option::Option<i64>) -> Self {
        self.limit = input; self
    }
    #[allow(missing_docs)] // documentation missing in model
    pub fn get_limit(&self) -> &::std::option::Option<i64> {
        &self.limit
    }
    /// ISO 8601 date-time.
    pub fn from_date(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.from_date = ::std::option::Option::Some(input);
        self
    }
    /// ISO 8601 date-time.
    pub fn set_from_date(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.from_date = input; self
    }
    /// ISO 8601 date-time.
    pub fn get_from_date(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.from_date
    }
    /// ISO 8601 date-time.
    pub fn to_date(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.to_date = ::std::option::Option::Some(input);
        self
    }
    /// ISO 8601 date-time.
    pub fn set_to_date(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.to_date = input; self
    }
    /// ISO 8601 date-time.
    pub fn get_to_date(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.to_date
    }
    /// Consumes the builder and constructs a [`ListPbAccountTransactionsInput`](crate::operation::list_pb_account_transactions::ListPbAccountTransactionsInput).
    pub fn build(self) -> ::std::result::Result<crate::operation::list_pb_account_transactions::ListPbAccountTransactionsInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(
            crate::operation::list_pb_account_transactions::ListPbAccountTransactionsInput {
                account_id: self.account_id
                ,
                offset: self.offset
                ,
                limit: self.limit
                ,
                from_date: self.from_date
                ,
                to_date: self.to_date
                ,
            }
        )
    }
}

